 ...
 * @since 12/4/14
 */
public class NewProductsCalculationNew {
    ...
    public static void main(String[] args) throws IOException, ParseException {
        ...
        loadData("/Users/abykovsky/export_2013-11-26_2013-11-28.csv", "/Users/abykovsky/export_2013-11-29_2013-12-02.csv",
                "30-NOV-13");
    ...
    private static void loadData(String windowCsvFile, String csvFile, String firstDate) throws IOException, ParseException {

        Set<Long> newProducts = filterNewProducts(firstDate);

        SimpleDateFormat format = new SimpleDateFormat("dd-MMM-yy");
        initWindow(windowCsvFile, format);
        ...
        int all = 0;
        Map<Long, MutableInteger> newProductsInTransactionMap = new HashMap<>();
        try (Reader in = new FileReader(csvFile)) {
                ...
                for (String s : record.get("QUANTITIES").split("\\|")) {
                    ...
                    if (s.length() > 0) {
                        ...
                        if (!isFreePrice(iter, actualPriceList) && newProducts.contains(key)) {
                            ...
                            newProductsInTransactionMap.putIfAbsent(key, new MutableInteger(0));
                            newProductsInTransactionMap.get(key).incrementAndGet();

                            if (!candidatesGenerator.getTopRecommendations(generateSet(key), K_TOP,
                                    transactionCount).isEmpty()) {
                                newProductsWithRecommendations.add(key);
                                recommSingleBuyCount += 1;
                            }
                        }
                        if (isFreePrice(iter, actualPriceList) && newProducts.contains(key)) {
            ...
            System.out.println("New count: " + newProductsInTransactionMap.size());
    ...
    private static void initWindow(String csvFile, SimpleDateFormat format) throws IOException, ParseException {
        int all = 0;
        try (Reader in = new FileReader(csvFile)) {
            Iterable<CSVRecord> records = CSVFormat.RFC4180.withHeader().parse(in);
            System.out.println("Start load data from \"" + csvFile + "\"");
